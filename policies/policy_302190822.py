...
class PolicySubmission(bp.Policy):
    ...
    def init_run(self):
        ...
        self.action = tf.reduce_sum(tf.mul(self.net_q, self.a), reduction_indices=1)
    ...
    def learn(self, reward, t):
        ...
        if t - 2 not in self.transition_dict or t - 1 not in self.transition_dict or t - 1 not in self.reward_dict:
            return

        # get the previous state, action, reward, state2, action2
        state_action1 = self.transition_dict.pop(t - 2)
        state1 = state_action1[0]
        action1 = state_action1[1]
        state_action2 = self.transition_dict[t - 1]
        state2 = state_action2[0]
        action2 = state_action2[1]
        old_reward = self.reward_dict.pop(t - 1)
        ...
        self.value_reward_dict[self.iteration_to_value[t-2]] = old_reward
        self.iteration_to_value.pop(t-2)
        ...
        # learn Deep-Q-Net
        self.learnQ(state1, action1, old_reward, state2, action2, t)

    def act(self, t, state, player_state):
        # encode state
        encoded_state = self.encode_state_generic(state, player_state, self.patch_radius)
        ...
        r, c = head_pos.move(bp.Policy.TURNS[player_state['dir']][action]) % state.shape

        # try not to kill yourself, in probability (won't get in here with default values)
        if np.random.rand() > self.suicide_prob and self.value_reward_dict.get(state[r, c], np.inf) <= self.minimal_reward:
            if not random_action:
                action = bp.Policy.ACTIONS[np.argsort(q)[0][1]]
            else:
                for act in bp.Policy.ACTIONS:
                    r, c = head_pos.move(bp.Policy.TURNS[player_state['dir']][act]) % state.shape
                    if self.value_reward_dict.get(state[r, c], np.inf) > self.minimal_reward:
                        action = act
                        break
        else:
            # try to improve action, in probability (low probability) for smart exploring
            pred_value = self.value_reward_dict.get(state[r, c], None)
            if pred_value is not None and np.random.rand() < self.eat_prob:
                for act in bp.Policy.ACTIONS:
                    r, c = head_pos.move(bp.Policy.TURNS[player_state['dir']][act]) % state.shape
                    if self.value_reward_dict.get(state[r, c], -np.inf) > pred_value:
                        action = act
                        break
        ...
        if (time.time() - self.t1) > self.max_time:
            ...
            self.iteration_to_value[t] = state[head_pos.move(bp.Policy.TURNS[player_state['dir']][action]) % state.shape]
            self.transition_dict[t] = (encoded_state, action)

            # reduce epsilon
            if self.epsilon > self.end_epsilon and t > self.observe_num:
    ...
    def encode_state_generic(self, state, player_state, radius):
        head_pos = player_state['chain'][-1]
